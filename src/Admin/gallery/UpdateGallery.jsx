...
import useUpdateGallery from "../../hooks/adminHook/adminGallery/useUpdateGallery";
const UpdateGallery = () => {
  const { preview, loading, navigate, handleImageChange, handleUpdateGallery } =
    useUpdateGallery();
  ...
  return (
    <>
      <div className="flex gap-2">
        <Sidebar />
        <HeroBar>
          <h1 className="text-[35px] p-2 font-bold">Update Gallery</h1>
          <button className="bg-blue-600 w-20 h-10 rounded-xs">
            <Link to={"/admin/dashboard/gallery"} className="text-white">
              Back
            </Link>
          </button>
